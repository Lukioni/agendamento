...
export const users = sqliteTable("users", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  name: text("name").notNull(),
  email: text("email").notNull().unique(), // login único
  password: text("password").notNull(), // depois tu renomeia pra passwordHash
  role: text("role").notNull().default("user"), // "admin" | "barber" | "user"
});
