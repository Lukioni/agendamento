...
CREATE TABLE `__new_bookings` (
	...
	FOREIGN KEY (`service_name`) REFERENCES `services`(`name`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`staff_id`) REFERENCES `staff`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`staff_name`) REFERENCES `staff`(`name`) ON UPDATE no action ON DELETE no action
);
